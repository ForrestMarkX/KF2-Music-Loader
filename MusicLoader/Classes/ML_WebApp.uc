Class ML_WebApp extends Object implements(IQueryHandler);

var WebAdmin webadmin;
var string ExtWebURL;
var int EditPageIndex,EditArrayPage;
var ML_WebAdmin_UI ExtAdminUI;
var MusicLoader MyMutator;
...
{
	menu.addMenu(ExtWebURL, "Music Loader Mod", self, "Modify settings of Music Loader.", -45);
}
...
final function AddConfigEditbox( WebAdminQuery q, string InfoStr, string CurVal, int MaxLen, string ResponseVar, string Tooltip, optional bool bSkipTrail )
{
	...
	S = "<TR><TD><abbr title=\""$Tooltip$"\">"$InfoStr$":</abbr></TD><TD><input class=\"textbox\" class=\"text\" name=\""$ResponseVar$"\" size=\""$Min(100,MaxLen)$"\" value=\""$CurVal$"\" maxlength=\""$MaxLen$"\"></TD>";
	if( !bSkipTrail )
		S $= "</TR>";
...
function handleExtMod(WebAdminQuery q)
{
	local int i,j,z;
	local string S;
	local delegate<ML_WebAdmin_UI.OnGetValue> GetV;
	local delegate<ML_WebAdmin_UI.OnSetValue> SetV;
	local bool bEditArray;

	if( ExtAdminUI==None )
	{
		ExtAdminUI = new (None) class'ML_WebAdmin_UI';
	...
	if( EditPageIndex<0 || EditPageIndex>=ExtAdminUI.ConfigList.Length )
	{
		...
		SendHeader(q,"Slots Mod Links page");
	...
	else
	{
		...
		if( bEditArray )
		{
			...
			q.response.SendText("<tr><td><form action=\""$webadmin.Path$ExtWebURL$"\"><input class=\"button\" name=\"GoToPage\" type=\"submit\" value=\"Return\"></form></td></tr>");
		}
		else
		{
...
defaultproperties
{
	ExtWebURL="/settings/MutSlotsMod"
	EditPageIndex=-1
}